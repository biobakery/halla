...
# 08/03
# associations = ['parabola', 'mixed', 'sine', 'step', 'log']
# metrics = ['dcor', 'nmi', 'nmi', 'dcor', 'spearman']
# noise_within = ['0.12', '0.25', '0.25', '0.33', '0.3']
# noise_between = ['0.12', '0.25', '0.3', '0.33', '0.2']
# iters = [5, 5, 5, 5, 5]
# fdrs = ['0.05', '0.1', '0.25', '0.5']

associations = ['parabola', 'mixed', 'sine', 'step', 'log']
metrics = ['dcor', 'nmi', 'nmi', 'dcor', 'spearman']
noise_within = ['0.12', '0.3', '0.4', '0.33', '0.3']
noise_between = ['0.12', '0.3', '0.4', '0.33', '0.2']
iters = [5, 5, 5, 5, 5]
fdrs = ['0.05', '0.1', '0.25', '0.5']

for fdr in fdrs:
    for i in range(len(associations)):
        for iter_i in range(0, 50):
            file = 'fin_08-03_%s_fdr%s_%d.csv' % (associations[i], fdr, iter_i)
...
#SBATCH -o fin_08-03_%s_fdr%s_%d_''' % (associations[i], fdr, iter_i) + '''%j.out  # File to which STDOUT will be written
#SBATCH -e ''' + 'fin_08-03_%s_fdr%s_%d_' % (associations[i], fdr, iter_i) + '''%j.err  # File to which STDERR will be written
...
''' + 'python examples/run_simulation.py -i 1 -n 100 -xf 500 -yf 500 --association %s --metric %s --fdr_alpha %s -nw %s -nb %s -o fin_08-03_%s_fdr%s_%d' % (
